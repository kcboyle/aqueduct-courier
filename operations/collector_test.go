...
var _ = Describe("Collector", func() {
	...
	It("collects opsmanager data and writes it", func() {
		...
		expectedD1Path := filepath.Join("opsmanager", d1.Name())
		d1Contents, d1Path := tarWriter.AddFileArgsForCall(0)
		expectedD2Path := filepath.Join("opsmanager", d2.Name())
		...
		expectedMetadataPath := filepath.Join("opsmanager", data.MetadataFileName)
	...
	It("returns an error when adding the metadata to the tar file fails", func() {
		tarWriter.AddFileStub = func(contents []byte, filePath string) error {
			if filePath == filepath.Join("opsmanager", data.MetadataFileName) {
	...
	Describe("credhub collection", func() {
		...
		It("collects credhub data and writes it", func() {
			...
			expectedCredhubDataPath := filepath.Join("opsmanager", chData.Name())
		...
		It("returns an error when adding credhub data to the tar file fails", func() {
			...
			tarWriter.AddFileStub = func(content []byte, filePath string) error {
				if filePath == filepath.Join("opsmanager", data.Name()) {
					return errors.New("tarring is hard")
				}
				return nil
			}
	...
	Describe("consumption collection", func() {
		...
		It("collects consumption data and writes it", func() {
			...
			chContentMd5 := base64.StdEncoding.EncodeToString(md5SumCH[:])
			consumptionData := usage.NewData(strings.NewReader(expectedConsumptionContents), "app-instances")
			...
			expectedConsumptionDataPath := filepath.Join("consumption", consumptionData.Name())
			...
			expectedMetadataPath := filepath.Join("consumption", data.MetadataFileName)
		...
		It("returns an error when the consumption collection errors", func() {
			consumptionDataCollector.CollectReturns(usage.Data{}, errors.New("collecting is hard"))
		...
		It("returns an error when reading the consumption data content fails", func() {
			failingReader := new(operationsfakes.FakeReader)
			failingReader.ReadReturns(0, errors.New("reading is hard"))
			failingData := usage.NewData(failingReader, "app-instances")
		...
		It("returns an error when adding consumption data to the tar file fails", func() {
			data := usage.NewData(strings.NewReader(""), "app-instances")
			consumptionDataCollector.CollectReturns(data, nil)
			tarWriter.AddFileStub = func(content []byte, filePath string) error {
				if filePath == filepath.Join("consumption", data.Name()) {
					return errors.New("tarring is hard")
				}
				return nil
			}
		...
		It("returns an error when adding the metadata to the tar file fails", func() {
			consumptionDataCollector.CollectReturns(usage.NewData(strings.NewReader(""), "app-instance"), nil)
			tarWriter.AddFileStub = func(contents []byte, filePath string) error {
				if filePath == filepath.Join("consumption", data.MetadataFileName) {
					return errors.New("tarring is hard")
				}
				return nil
			}
